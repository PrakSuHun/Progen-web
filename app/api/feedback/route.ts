...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, phone, score_overall, score_content, score_practice, score_network, good_tags, good_points, bad_tags, bad_points, would_return, join_interest } = body
    ...
    if (!crewId && !guestId) {
      return NextResponse.json({ message: '사전 신청 또는 출석 정보를 찾을 수 없습니다' }, { status: 404 })
    }
    ...
    if (error) {
      if (error.code === '23505') {
        return NextResponse.json({ message: '이미 피드백을 제출하셨습니다' }, { status: 409 })
      }
      throw error
    }

    return NextResponse.json({ message: '피드백이 저장되었습니다', data })
  } catch (error) {
