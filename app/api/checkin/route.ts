...
export async function POST(request: NextRequest) {
  try {
    ...
    if (walkin) {
      ...
      if (error) {
        if (error.code === '23505') {
          return NextResponse.json({ message: '이미 출석체크되었습니다' }, { status: 409 })
        }
        throw error
      }

      return NextResponse.json({ message: '현장 등록 + 출석 완료', name, data })
    } else {
      let crewId: string | null = null
      let guestId: string | null = null
      ...
      if (!crewId && !guestId) {
        return NextResponse.json({ message: '사전 신청 정보를 찾을 수 없습니다' }, { status: 404 })
      }
      ...
      if (registrations.status === '출석완료') {
        return NextResponse.json({ message: '이미 출석체크되었습니다', name }, { status: 409 })
      }
      ...
      return NextResponse.json({ message: '출석 완료', name })
    }
  } catch (error) {
