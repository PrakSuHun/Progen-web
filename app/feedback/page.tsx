...
export default function FeedbackPage() {
  ...
  const stepIndicator = (current: number) => (
    <div className="flex gap-1.5 mb-6">
      {[1, 2, 3].map((s) => (
        <div key={s} className={`h-1 rounded-full flex-1 transition-colors ${s <= current ? 'bg-violet-500' : 'bg-[#e0e0e0]'}`} />
      ))}
    </div>
  )
        ...
        <div className="max-w-lg mx-auto">
          ...
          {step === 0 && (
            <div className="bg-white border border-[#eee] rounded-2xl p-6 md:p-8 text-center space-y-5">
              <div className="w-16 h-16 rounded-2xl bg-violet-100 text-violet-500 font-black text-2xl flex items-center justify-center mx-auto">?</div>
            ...
            <div className="bg-white border border-[#eee] rounded-2xl p-5 md:p-8 space-y-5">
              ...
              <div>
                ...
                <textarea value={formData.good_points} onChange={(e) => { setFormData({ ...formData, good_points: e.target.value }); if (errors.good_points) setErrors({}) }}
                  ...
                  className={`w-full px-4 py-3 bg-white border rounded-xl text-[#111] placeholder-[#aaa] focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/10 transition h-32 text-base ${errors.good_points ? 'border-red-400' : 'border-[#e0e0e0]'}`}
                />
            ...
            <div className="bg-white border border-[#eee] rounded-2xl p-5 md:p-8 space-y-5">
              ...
              <div>
                ...
                <textarea value={formData.bad_points} onChange={(e) => { setFormData({ ...formData, bad_points: e.target.value }); if (errors.bad_points) setErrors({}) }}
                  ...
                  className={`w-full px-4 py-3 bg-white border rounded-xl text-[#111] placeholder-[#aaa] focus:outline-none focus:border-violet-500 focus:ring-2 focus:ring-violet-500/10 transition h-32 text-base ${errors.bad_points ? 'border-red-400' : 'border-[#e0e0e0]'}`}
                />
          ...
          {step === 3 && (
            <div className="bg-white border border-[#eee] rounded-2xl p-5 md:p-8 space-y-5">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer p-4 bg-[#fafafa] border border-[#eee] rounded-xl hover:border-violet-200 transition">
                  <input type="checkbox" checked={formData.would_return} onChange={(e) => setFormData({ ...formData, would_return: e.target.checked })} className="w-5 h-5 accent-violet-500" />
                  <span className="text-[#333] text-sm">다음 행사에도 참여하고 싶습니다</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer p-4 bg-[#fafafa] border border-[#eee] rounded-xl hover:border-violet-200 transition">
                  <input type="checkbox" checked={formData.join_interest} onChange={(e) => setFormData({ ...formData, join_interest: e.target.checked })} className="w-5 h-5 accent-violet-500" />
          ...
          {step === 4 && (
            <div className="bg-white border border-[#eee] rounded-2xl p-6 md:p-8 text-center space-y-5">
              ...
              {formData.join_interest && (
                <div className="bg-violet-50 border border-violet-100 p-5 rounded-xl">
                  <p className="text-violet-600 text-sm mb-3 font-semibold">PROGEN에 관심을 가져주셔서 감사해요!</p>
