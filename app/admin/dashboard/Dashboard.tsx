...
export function AdminDashboard({ kind = 'regular' }: { kind?: 'regular' | 'event' }) {
  ...
  const loadEvents = async (cohortParam?: string) => {
    ...
    if (cohortParam) params.set('cohort_id', cohortParam)
    if (isEventKind) { params.set('kind', 'event'); params.set('cohort_id', 'all') }
  ...
  const handleEventChange = (newEventId: string) => {
    // 드롭다운 맨 아래 바로가기 옵션 — 행사 어드민 ↔ 이벤트 어드민 이동
    if (newEventId === 'goto-event') { window.location.href = '/admin/event'; return }
    if (newEventId === 'goto-dashboard') { window.location.href = '/admin/dashboard'; return }
    ...
    <div className="h-screen bg-slate-100 flex flex-col overflow-hidden">
      ...
      <header className="bg-sky-700 px-4 md:px-6 py-2.5 md:py-3 flex items-center justify-between flex-shrink-0">
        <div className="flex items-center gap-2 md:gap-4 flex-1 min-w-0">
          <h1 className="text-sm md:text-lg font-bold text-white shrink-0">PROGEN{isEventKind && <span className="ml-1.5 text-[10px] md:text-xs font-bold text-amber-300 align-middle">이벤트</span>}</h1>
          {!isEventKind && cohorts.length > 1 && (
            <select
              value={cohortFilter}
          ...
          {events.length > 0 && (
            ...
            >
              ...
              {!isEventKind && <option value="crew-all">PROGEN 1기 크루</option>}
              <option value={isEventKind ? 'goto-dashboard' : 'goto-event'}>
                {isEventKind ? '→ 행사 바로가기' : '→ 이벤트 바로가기'}
              </option>
            </select>
          )}
        </div>
        <div className="flex gap-2 shrink-0">
          {/* 행사↔이벤트 이동은 행사 드롭다운 맨 아래 「바로가기」 옵션으로 */}
          {!isEventKind && (
