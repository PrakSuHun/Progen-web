...
> **마지막 최신화**: 2026-09-23 (**이벤트 어드민 탭 축소 + 모바일 가로 스크롤 수정 + 신청자 탭 동석자 노출**). ① `/admin/event`(kind=event)에서 **출석·보증금 탭 제거** — `Dashboard.tsx`에 `visibleTabs`(크루 전체 모드=분석·신청자 / 이벤트=**신청자·팀배정·분석 순**) 신설, 이벤트 어드민 기본 탭은 신청자(`useState` 초기값 분기), renderCheckin/renderDeposit도 `!isEventKind` 가드. ② **모바일 가로 스크롤 수정**(행사·이벤트 어드민 공통) — 탭 콘텐츠 루트 4곳과 출석 탭 컬럼 내부 리스트·팀배정 스크롤 영역에 `overflow-x-hidden` 추가(`overflow-y-auto`만 있으면 x축이 암묵 auto가 되어 자식 오버플로 시 좌우 스크롤 발생), 출석 탭 PersonCard 래퍼 `flex-1`에 `min-w-0`(flex 자식 min-width:auto 때문에 긴 내용이 줄어들지 않던 것). ③ **신청자 탭 모바일 접힌 카드에 동석자(companion) 노출** — 종전엔 카드를 펼쳐야만 보였는데, 학교·학년·성별 줄 아래 「🤝 같이 신청 OOO」 violet 배지로 항상 표시(행사 신청자 모드 한정, 데스크톱 테이블 동석자 열은 종전대로). 이전: 2026-09-21 (**피드백 귀속 행사 로직 신설 — 행사 후 7일까지 직전 행사로 집계**). 피드백이 행사가 끝난 뒤 들어오면 `getActiveEventId()`(미래 행사 우선)가 다음 행사로 잘못 연결하던 버그 수정 — 실사례: 9/21 제출된 포도용 피드백 2건(김윤서·박수훈)이 10/1 「AI STUDY CREW 이벤트」(is_event 프로모션 row)로 붙어 분석 탭에 안 보였음(9/19 행사로 소급 정정 완료). **`lib/get-active-event.ts`에 `getFeedbackEventId()` 신설**: ① 1순위 — 가장 최근에 지난 행사(당일 포함)가 `FEEDBACK_WINDOW_DAYS=7`일 이내면 그 행사 ② 2순위 — 가장 가까운 미래 행사 ③ **is_event=true(프로모션 이벤트) row는 후보 제외**. `/api/feedback`(익명)·`/api/grape-feedback`(포도용) 둘 다 이 헬퍼로 교체(신청·체크인 등 다른 경로는 종전 getActiveEventId 그대로). 같은 날 이전: (**포도용 피드백 신설 — /grapefeedback + 분석 탭 「포도용 피드백」 섹션**). 일반 행사 피드백(익명 /feedback)과 별도로 포도 전용 **기명** 피드백 수집: ① `grape_feedbacks` 테이블 신설(id·event_id FK·name·q1~q3·created_at, RLS 0정책 default-deny, 마이그레이션 `2026-09-21_grape_feedbacks.sql` 라이브 적용). ② 공개 폼 **`/grapefeedback`** — 이름 + 3문항(1. 생명과 친해질 수 있었나요? 어떤 부분이 도움이 되었나요? / 2. 애로사항이 있었나요? / 3. 생명을 2차 만남 혹은 이후 계획이 있으신가요?)이 **한 화면**에 뜨고 바로 제출(전 항목 필수). ③ 공개 API `POST /api/grape-feedback` — 활성 행사(`getActiveEventId`, 날짜 기반)에 연결 INSERT. ④ 분석 탭 「피드백 원문」 카드 바로 아래 **「🍇 포도용 피드백」 카드**(violet 톤, 이름+3문답, 응답 없으면 안내 문구) — `full-stats` section3에 `grape_responses` 추가(crew-all 모드는 빈 배열). 검증: 빌드 통과 + 로컬 dev로 저장/필수값 400/event_id 연결 확인 후 테스트 행 정리. 이전: 2026-09-19 (**팀배정 탭 팀명 클릭 → 팀 순서 교체 드롭다운**). 팀 카드의 팀명 클릭 시 인라인 이름 편집(input)이던 것을 **전체 팀 목록 드롭다운**으로 교체 — 다른 팀(예: 12팀)을 선택하면 두 팀 멤버 전원을 서로 맞바꿈(`handleSwapTeams`, 낙관적 UI + 멤버별 `assign-team` 호출, 빈 팀과의 교체도 가능 = 해당 슬롯으로 이동). 팀명 옆 ⇄ 아이콘 표시. **팀명 자유 rename 기능(`handleRenameTeam`)은 제거됨** — 복원하려면 git 히스토리 참조. 이전: 2026-09-18 (**4번 사전 공지 템플릿 「행사 사전 공지(사본)」으로 교체 + 행사 정보 탭 칸 정리**). 솔라피 신규 「4. 행사 사전 공지(사본)」(`KA01TP260915043622989u7WTWk06rij`, APPROVED — API 실조회 검증) 채택: `EVENT_D1_NOTICE` 코드 교체 + **변수 축소 — 입장시간·진행내용 삭제**(잔여: 이름·프로그램명·일시·장소·준비물 + `#{url}` 버튼), `varsEventD1Notice`에서 두 변수 제거(준비물 1차/2차 materials2 분기는 유지). 설정 「행사 정보」 탭에서 **「입장 시간」·「당일 진행」 입력칸 제거**(일시→장소→준비물→채팅방 순으로 정리) — `events.entry_time`/`program_detail` 컬럼·event-settings API 배관은 값 보존 위해 유지(편집 UI만 제거, 8번 등 타 템플릿도 미사용). 주의: dedup·1차/2차 발송횟수가 template_code 기준이라 **구 코드로 발송된 이력은 새 코드 기준 0회로 리셋** — 9/19 행사에서 이미 1차 공지를 받은 사람도 새 코드 1차 미발송으로 잡히니 명단 체크박스로 선별 발송할 것. 이전: 2026-09-17 (**크루 추가 시 학번 자동 복사**). 어드민 「+ 크루 추가」(`add-registration`)가 크루 프로필의 `student_number`를 신청 건(`event_registrations.student_number`)에 복사해서 INSERT — 신청자 탭 학번 표시가 신청 건 기준이라 크루 추가로 넣은 사람은 학번이 비어 보이던 문제. 9/19 신청 건 소급 1명(강준혁). 누적 크루 화면에 학번 노출은 운영자 결정으로 **하지 않음**(학번은 행사 신청 건 관리 용도). 같은 날 이전: (**크루 학번 저장(crew_members.student_number)**). 행사 사전신청 폼의 학번이 `event_registrations.student_number`(신청 건 단위)에만 남고 크루 프로필엔 없던 것 개선 — ① `crew_members.student_number TEXT` 신설(마이그레이션 `2026-09-17_crew_student_number.sql`, 라이브 적용). ② `/api/event-reg` mode=crew가 신청 시 입력된 학번을 크루 레코드에도 update(매 신청마다 최신 값 갱신, 빈 값은 무시). ③ 기존 신청분 백필 — 크루 신청 건의 학번을 최신 신청 우선으로 26명 반영. 크루 지원 폼(/apply)에는 학번 입력란 없음(운영자 결정) — 행사 신청을 한 번도 안 한 크루는 여전히 학번 null. 같은 날 추가: 9/19 게스트 신청자 **김민종(#276)·고경석(#277)도 동일 방식으로 포도 편입**(크루 생성 — 가입일 3/28 통일값·학번 동기화, 신청 건 크루 전환, 게스트 삭제. 둘 다 크루 명단에 원래 없어 게스트 폼 크루번호 차단에 안 걸렸던 케이스). 같은 날 이전: (**포도 가입일 3/28 통일 + 9/19 포도 3명 편입 — DB 데이터만, 코드 무변경**). ① 포도(is_member=true) 47명 전원의 `crew_members.created_at`을 **2026-03-28 00:00 KST**(`2026-03-27T15:00:00Z`)로 통일 — 포도가 게스트→크루 **전환 집계(full-stats `crew_conversion_count`, `created_at >= eventDate` 조건)에 절대 안 잡히게** 하려는 운영자 요청. 부수효과(의도됨): 김양균(5/16)·한수빈·이소연(6/27) 과거 게스트 신청 3건이 `isCrewAtRegistration` 판정상 크루로 바뀌어 보증금 탭 미입금 목록에서 빠짐(포도라 보증금 비대상). ② 9/19 행사 신청자 **신채경·전서희·정새준 포도 편입** — 신채경은 기존 크루 #271에 is_member=true만 지정, 전서희(#274 충남대)·정새준(#275 한남대)은 게스트 신청이어서 크루 신규 생성(포도) + 9/19 신청 건 crew_id 전환 + 게스트 레코드 삭제(권미진 정정과 동일 방식). 전서희·정새준은 게스트 접수(1번) 알림톡을 이미 수신(보증금 계좌 안내 포함) — 입금 불필요 별도 안내 권장. 이전: 2026-09-16 (**사전 공지 대상 미입금 제외**). 사전 공지(4번) 1차/2차 대상을 「노쇼확정 제외 전원」→「크루 + 입금된 게스트」(구 확정 명단 기준)로 변경 — event-settings 명단(eligibleForD1=eligibleForConfirm)·send-alimtalk-batch d1 타깃 동일 적용. 미입금 게스트는 명단에서 제외되고 입금 시 자동 편입, 미입금 안내는 보증금 탭 11번→5번 시퀀스가 담당. 같은 날 이전: (**사전 공지 준비물 1차/2차 분리(events.materials2)**). 사전 공지 회차별로 준비물 문구가 달라 분리 — `events.materials2 TEXT` 신설(마이그레이션 `2026-09-16_event_materials2.sql`, 라이브 적용). `varsEventD1Notice(ev, name, round)`가 2차면 materials2 사용(비면 1차 materials fallback), batch d1이 round 전달. 설정 「행사 정보」 탭 준비물 칸에 **1차/2차 공지 토글**(두 값 모두 저장 버튼으로 함께 저장). 이전: 2026-09-15 (**이벤트 서브시스템 신설 — /admin/event + /event-reg/event**). 프로모션성 「이벤트」를 행사와 분리 집계: ① `events.is_event BOOLEAN` 신설(마이그레이션 라이브 적용) + `getActivePromoEventId()`(is_event=true 날짜 기반). ② `/api/admin/events` — 기본 GET은 이벤트 제외, `?kind=event`는 이벤트만(+defaultEventId도 이벤트 기준), POST `is_event:true`면 is_public=false로 생성. ③ 대시보드를 `Dashboard.tsx` 공용 컴포넌트(kind prop)로 분리 — `/admin/dashboard`(regular)·**`/admin/event`(event 전용 — 2026-09-23부터 출석·보증금 탭 제외 3탭: 신청자·팀배정·분석 순, 기본 탭=신청자)** 래퍼 2개. ④ 신청폼 **`/event`** — 게스트 폼에서 보증금 안내·환불계좌 제거(companion·학번 유지), `kind:'event'` 제출 → event-reg가 이벤트 행사로 연결·알림톡 미발송·크루번호 차단 스킵. 검증: 실DB 테스트 이벤트로 신청→집계→목록 분리→알림톡 미발송 전 구간 확인 후 정리. **이벤트 row 생성은 아직 수동**(POST API 또는 DB) — 4-1절 is_event 참고. 같은 날 이전: (**확정 알림톡 UI 제거**). 설정 → 알림톡 발송 탭의 「참석 확정 알림톡」 일괄발송 섹션 삭제(운영 방침: 확정 문자 폐지, 사전 공지 1차가 대체). **서버(send-alimtalk-batch template=confirm)와 템플릿·명단 API는 그대로 남아 있어 UI 복원만으로 되살릴 수 있음.** 행사정보 탭 준비 배너 문구도 '오픈채팅 버튼 동작' 관점으로 변경. 같은 날 이전: (**사전 공지 1차/2차 발송 체계**). 운영 방침: 확정(3번) 문자 생략, 사전 공지(4번)를 **1차(확정 안내 대체)·2차(행사 전날 리마인드)** 두 번 발송. ① `send-alimtalk-batch` template=d1에 `round`(1|2) 파라미터 — 중복 방지를 alreadySent(1회성)에서 **발송 로그 횟수 >= round 판정**으로 교체(2차는 1회 받은 사람까지 발송, 1차 안 받은 사람도 2차 명단에 포함됨). ② `event-settings` GET이 d1 recipients에 `count`(발송 횟수) + pending.d1.sent2 반환. ③ 설정 탭 사전 공지 섹션에 **1차/2차 토글** — 회차 선택 시 해당 회차 미발송자 자동 선택, 명단 회색 표시도 회차 기준. 확정(3번) 일괄발송 섹션은 UI에 그대로 남김(방침 재변경 대비, 사용 안 하면 무해). 같은 날 이전: (**알림톡 행사명 표기 커스텀(events.program_name_text)**). `#{프로그램명}`도 datetime_text와 동일 패턴으로 오버라이드 가능 — `events.program_name_text` 신설(마이그레이션 라이브 적용) + `programNameOf(ev)` 헬퍼로 알림톡 발송 전 경로 통일(solapi vars 4종 + checkin·send-individual·send-cancel·send-deposit-reminder·sms-hook·batch change·test-alimtalk·toggle-deposit) + 설정 「행사 정보」 탭 최상단 「알림톡 행사명 표기 (선택)」 입력칸. 비우면 title 그대로. 이전: 2026-09-09 (**알림톡 프로그램명 낫표 제거**). `programLabel()`이 `#{프로그램명}`을 `「행사명」`으로 감싸던 것을 행사명 그대로로 변경(전 템플릿 공통 적용). 같은 날 이전: (**알림톡 템플릿 12종 전부 검수 승인 확인**). 솔라피 등록 12종 전량 APPROVED — 접수(크루)·입금완료(게스트)도 승인되어 크루 신청·입금 확인 시 자동 발송이 실제로 나가기 시작함. UI '검수중' 표시 제거. 미등록 잔여: 노쇼 경고(운영진 재등록 예정 — 등록되면 NOSHOW_WARNING 코드 교체 필요, 크루 자격 박탈은 삭제 상태 유지). 같은 날 이전: (**4번 공지 템플릿 교체 — 「행사 사전 공지」**). 구 「행사 전 공지」가 솔라피에서 삭제되고 신규 「4. 행사 사전 공지」(`KA01TP260907034359294lbAuExhWRIx`)가 승인되어 `EVENT_D1_NOTICE` 코드 교체 + 설정 탭·테스트 드롭다운 문구를 '행사 사전 공지'로 갱신(변수는 진행내용 포함 동일 — 어제 varsEventD1Notice 누락 수정으로 이미 대응됨). 참고: dedup이 코드 기준이라 구 코드로 발송된 이력은 새 코드 기준 미발송으로 집계됨. 같은 날/이전: (**설정 모달 알림톡 탭 모바일 가로 오버플로 수정** — 테스트 select/input min-w-0 + overflow-x-hidden, 드롭다운 라벨 솔라피 제목과 동일화). 이전: 2026-09-08 (**씨엔유케어 교차 표시 신설**). 운영자의 별도 서비스 **씨엔유케어**(Supabase `jjuevppixlksagbsktxo`) 행사 명단과 전화번호가 **완전히 일치하는 비포도 인원**을 어드민에서 주황색으로 표시. ① 신설 **`lib/cnucare.ts`** — `getCnucareEventsByPhone()`: CNUcare `event_attendees`(+`events` 조인, ~330행)를 anon key로 전량 조회해 전화번호(숫자만)→참여행사 맵 생성, `annotateCnucare(list, map)`: 포도(is_member) 제외하고 `cnucare_events:[{name,date}]` 부착. env `CNUCARE_SUPABASE_URL`/`CNUCARE_ANON_KEY`(.env.local + Vercel Production 등록) 미설정 시 no-op. ② `dashboard-data`·`members-list`(양 모드) 응답에 부착. ③ UI: **팀배정·체크인 탭 `PersonCard`** — 이름 주황 + 「CNU」 배지(hover 시 참여 행사 tooltip) / **신청자 탭** — 모바일 카드 이름 주황+주황점(포도>CNU>첫참여 우선순위), 데스크톱 테이블 이름 주황+「CNU」 배지, **모바일 상세(확장)에 「씨엔유케어 참여 이력」 박스(행사명+날짜 목록)**. 검증: 실데이터 교차 11건 확인(박한빈·최미성·권미진·정다현·김승완 등 — 대둔산 서포터즈 2기·원회원·완주군 산촌체험). 같은 날 이전: (**노쇼 카운트 외부 행사 한정 집계**). `noshow_count`가 내부 회차 노쇼확정에도 증가하던 것을 **외부(is_public=true) 행사에서만 증감**하도록 `update-status` 수정(내부 회차는 노쇼확정 상태 표시만 되고 카운트 무반영 — 프로젝트 인원의 결석은 수료관리 영역이라 의미가 다름). 라이브 데이터도 '외부행사 노쇼확정 수' 기준으로 전면 재계산 — 8명 정정(윤혜은·한의정·김재욱·배지애·우승균·이주진·김가현·배수찬, 전원 8/21 내부 매칭데이 노쇼분이라 0으로. 우승균은 삭제된 신청 잔존치 포함 2→0). 같은 날 이전: (**솔라피 템플릿 개편 반영 — 접수(크루)·입금완료(게스트) 신설 연결, 노쇼 2종 삭제됨**). 운영진이 솔라피 템플릿을 정리(신규 번호 체계: 1 접수 게스트/크루 → 2 입금완료 → 3 확정 게스트/크루 → 4 공지 → 5 미입금 안내). 코드 반영: ① **`EVENT_REG_RECEIVED_CREW`**(`KA01TP260908051130190N5evwOwCriP`, ⏳검수중, 변수 게스트 접수와 동일) 신설 — `/api/event-reg` mode=crew가 신청 즉시 발송("접수 완료, 확정은 추후" 안내라 수동확정 방침과 부합). ② **`DEPOSIT_RECEIVED`**(`KA01TP2609080453491757bL3GWWKncA`, ⏳검수중, 변수 이름·프로그램명) 신설 — **입금 확인 시(sms-hook 자동 + toggle-deposit 수동) 자동 발송**("입금완료, 확정은 추후 공지"). 확정(3번)은 계속 설정 탭 수동. ③ 🧪 테스트 드롭다운 새 번호 체계로 재정렬 + 신규 2종 추가. ④ **노쇼 경고(9)·크루 자격 박탈(10) 템플릿이 솔라피에서 삭제됨** — 출석 탭 「노쇼경고」 버튼(send-individual-alimtalk type=noshow)은 이제 발송 실패함, 버튼 제거/템플릿 재등록 방침 미정(12번 표 참고). 테스트 드롭다운에선 제외. ⑤ 4번 공지 **사본**(`KA01TP260907034359294lbAuExhWRIx`, ⏳검수중) — 변수 셋은 기존 4번과 동일(순서만 변경, 운영진 문구 추가 예정), **승인되면 EVENT_D1_NOTICE 코드만 교체**(그 전까진 구 4번 유지). 이 과정에서 **기존 버그 발견·수정: varsEventD1Notice가 템플릿에 있는 `#{진행내용}` 변수를 누락한 채 발송하고 있었음** → 진행내용 추가(빈 값이면 '채팅방 공지 참고' fallback). ⏳검수중 2종은 승인 전 발송 시 solapi 거부(로그 failed) — 검수 통과 시 별도 코드 수정 없이 발송되기 시작함. 같은 날 이전: (**확정 알림톡 전면 수동 전환 — 자동 발송 4지점 제거**). 운영 방침 변경: 입금 확인·크루 신청은 자동 처리하되 **참석 확정(2번) 알림톡은 어디서도 자동 발송하지 않음** — 운영진이 설정 → 알림톡 발송 탭 **일괄발송 명단(원래부터 크루 + 입금 게스트만 포함, 미입금 제외)에서 확인 후 수동 발송**. 제거된 자동 발송 4지점: ① `sms-hook`(자동 입금 확인 시 — 이제 입금 처리 + 관리자 푸시만, 푸시 문구에 "설정 탭에서 발송" 안내) ② `toggle-deposit`(수동 입금 토글 시 — 상태만 변경, 대시보드 confirm 문구·토스트 갱신) ③ `/api/event-reg` mode=crew(신청 즉시 발송 제거 — 크루는 이제 확정 미발송 명단에 잡힘) ④ `add-registration`(크루 추가 시 — 항상 alimtalk:'pending' 반환). 6번 표의 2번 게스트/크루 행 "발송 시점"은 이제 **send-alimtalk-batch(설정 탭) + 🧪 테스트만** 유효. 재자동화하려면 이 4지점에 발송 블록 복원(git 히스토리 `114d2a2` 이전 참조). 같은 날 이전: (**알림톡 일시 표기 커스텀(events.datetime_text)**). 알림톡 `#{일시}`가 event_date 자동 포맷이라 "오후 2시~5시" 같은 상세 표기가 불가하던 것 개선 — `events.datetime_text TEXT` 신설(마이그레이션 `2026-09-08_event_datetime_text.sql`, 라이브 적용) + `lib/solapi.ts` `eventDateLabel(ev)`(입력값 우선, 비면 자동 포맷)로 1·2크루·4번 템플릿의 `#{일시}` 일원화 + 설정 「행사 정보」 탭 최상단에 「알림톡 일시 표기 (선택)」 입력칸(event-settings GET/POST가 처리). 행사정보 탭 준비 배너 문구도 신 조건(채팅방 링크만)으로 정정. 같은 날 이전: (**설정 모달 「알림톡 테스트」 신설**). 설정 → 알림톡 발송 탭 하단에 🧪 테스트 박스 — 템플릿 12종 드롭다운 + 전화번호 입력 + 발송 버튼으로 아무 번호에나 1건 테스트 발송(신규 라우트 `POST /api/admin/test-alimtalk`, 6번 표 참고). 같은 날 이전: (**알림톡 템플릿 3종 신규 교체(1번·2번게스트·2번크루) + 확정 발송조건 완화 + 11번 승인 확인**). 운영진이 솔라피에 새로 등록·승인받은 템플릿으로 교체: ① **1번 신청접수(게스트)** `KA01TP260906103329534uNkFHnMsp0a` — 변수 이름·프로그램명·일시(장소 삭제). ② **2번 확정(게스트)** `KA01TP260906103529205rx0U0oQ90Q7` — 변수 이름·프로그램명·url 3개(신설 `varsEventConfirmedGuest`). ③ **2번 확정(크루)** `KA01TP260906112122409DXPjnrJCl2k` — 변수 이름·프로그램명·일시·url(신설 `varsEventConfirmedCrew`). 신규 확정 템플릿은 장소·준비물 등 상세 변수가 없어져 **발송 조건을 `eventConfirmReady`(5필드) → `confirmChatReady`(채팅방 링크만)로 완화** — 구 `eventConfirmReady`·`varsEventConfirmed`는 삭제(콜사이트 6곳 교체: event-reg·add-registration·toggle-deposit·sms-hook·send-alimtalk-batch·event-settings). batch confirm 게이트 문구도 "채팅방 링크 입력"으로 변경. **sms-hook 자동 입금 확인 시에도 신규 게스트 확정 템플릿 자동 발송**(요청사항). 11번 DEPOSIT_REMINDER는 승인 확인됨(코드 무변경, 이제 실발송 가능). 템플릿 상태·변수는 솔라피 API로 실조회 검증(3종 모두 APPROVED). 주의: 확정 dedup이 template_code 기준이라 **구 템플릿으로 이미 확정 받은 사람은 새 코드 기준 '미발송'으로 집계** — 9/19 행사에서 알림톡 발송 탭 일괄발송 시 중복 수신 가능성 있으니 명단 체크박스로 선별 발송 권장. 같은 날 이전: (**보증금 입금 SMS 웹훅 신설 — 공기계 문자 포워딩 기반 반자동 입금확인**). 팝빌 철회(아래 항목) 후 채택한 대안: 하나은행 입출금 문자통지(월 800원)를 받는 **안드로이드 공기계에 SmsForwarder 앱**을 상주시켜 문자 수신 즉시 **`POST /api/sms-hook`**으로 전달. ① 신규 라우트 `app/api/sms-hook/route.ts`(공개 경로, `SMS_HOOK_SECRET` 헤더 `x-sms-secret` 또는 body.secret 자체 인증, env 미설정 시 503 비활성). 처리 규칙: 출금·비입금 문자 무시 → 금액이 **정확히 5,000원**(`입금 N원` 패턴 우선) → **미입금 게스트(노쇼확정 제외, 행사일 14일 윈도우) 이름이 문자에 포함**되면 자동 `deposit_status='입금'`(조건부 update로 중복 문자 멱등) + 2번 확정 알림톡(toggle-deposit과 동일 조건) + 관리자 웹푸시 "✅ 자동 확인". **매칭 실패·동명이인은 자동 처리 안 하고 관리자 웹푸시로 수동 확인 요청**(조용한 누락 방지 — 기존 `lib/push.ts` sendPushToAdmins 재사용). `body.test===true`면 부작용 없이 매칭 결과만 반환(공기계 설정 검증용). ② env `SMS_HOOK_SECRET` 추가(.env.local + Vercel Production 등록 완료, 2026-09-08 배포됨). ③ 검증: 로컬 dev + 실DB로 401/출금무시/금액불일치/실명 매칭(9/19 행사 황정은 test모드)/14일 윈도우 제외(4/11 강태준) 전 경로 확인. **남은 작업(운영자)**: 하나은행 기업뱅킹 문자통지 신청(수신번호=공기계) + 공기계 SmsForwarder 설정 + Vercel env 등록 — 12번 표 참고. 같은 날 이전: (**보증금 입금 자동확인(팝빌 계좌조회 연동) 구축 후 전면 철회 — 코드 무흔적 제거**). 9/7에 팝빌 계좌조회(EasyFinBank) API 연동을 구현(lib/popbill-bank.ts + deposit-sync 라우트 2개 + 보증금 탭 자동확인 버튼 + vercel.json 크론 + popbill 의존성)했으나, **팝빌 API 견적서 수령 결과 계좌조회 API는 웹용 정액제(월 5,500원)와 별개 상품으로 계좌 1개당 월 100,000원** — 규모 대비 과해 채택 취소. 관련 코드·의존성·env 자리(POPBILL_*·CRON_SECRET)를 전부 제거해 9/6 상태로 복원(빌드 통과 확인). 상세 검토 결과와 대체 운영 방식은 **12번 표 「보증금 자동 입금 확인」** 항목 참고. 이전 변경: 2026-09-06 (**출석 탭 자동문자 토글 노출조건 완화**). 자동문자 ON/OFF 버튼이 활성 행사(`selectedEventId === activeEventId`)에서만 보이던 것을 **실제 행사 선택 시 항상 노출**로 변경 — 내부 회차(9/18 최종PT)가 공개 행사(9/19 AI시대 대학생으로 살아남기)보다 앞 날짜라 어드민 기본선택과 날짜기반 활성 행사가 어긋나 토글이 사라져 보이던 문제. 플래그는 행사별 저장 + 발송은 당일 활성 행사 체크인에서만 일어나 미리 켜둬도 안전. 같은 날 이전: (**알림톡 발신번호 변경**) — 12번 표 「알림톡 발신번호」 항목 참고. 같은 날 이전: (**보증금 탭 「미입금 알림」 버튼 신설 — 신규 11번 템플릿**). 미입금 게스트에게 보증금 입금 안내를 보내는 버튼 — 취소 알림 왼쪽, **`deposit_status==='미입금'`인 행에만 노출**(amber 텍스트). 운영진이 솔라피에 **「5. 보증금 미입금 안내」 전용 템플릿 직접 등록**(`KA01TP260906071133136qAave0oFtch`, 변수 이름·프로그램명 2개, 본문: 입금 계좌 하나은행 660-910011-22904 + 참석 시 전액 반환 안내) — **현재 검수중(INSPECTING), 통과 전엔 발송 시 솔라피 거부**(버튼은 "발송 실패" 토스트). `lib/solapi.ts` `ALIMTALK.DEPOSIT_REMINDER` 추가. 새 라우트 **`POST /api/admin/send-deposit-reminder`**(`{registration_id}`) — 서버에서 게스트 여부 + 미입금 상태 재검증(아니면 400/409) 후 발송, alimtalk_logs 기록. **취소 알림 버튼과 통합 — 총 3회 시퀀스**: 1·2회차=미입금 알림(11번, `MAX_REMINDERS=2` 서버 409 방어), 3회차=취소 알림(5번). 별도 컬럼 없이 alimtalk_logs의 registration_id·템플릿·status='sent' 카운트로 판정. `dashboard-data`가 `deposit_reminder_count`(11번 횟수) + `cancel_notice_sent`(5번 발송 여부) 반환. UI(`DepositRow` `onOpenAlert` → 페이지 `depositAlert` state + `Modal`): 미입금 행에 단일 버튼 — 「미입금 알림 n/2」(amber) → 2회 소진 시 「취소 알림 (3회차)」(red) → 취소 발송 후 회색 「취소 알림 완료」. 버튼 클릭 시 **커스텀 팝업**(window.confirm 대체): 현재 발송 현황 + 단계별 발송 버튼 + **「바로 취소 알림 보내기」**(1·2회차 단계에서도 미입금 알림 건너뛰고 즉시 취소 알림 발송 가능). 발송 성공 시 팝업 닫고 `fetchAll()` 갱신. `sendDepositAlert(registration_id, 'reminder'|'cancel')` 공용 핸들러(종전 `handleSendCancelAlimtalk`/`handleSendDepositReminder` 대체). 참고: 1번(신청 접수) 템플릿은 APPROVED 확인됨(본문에 보증금 계좌·3일 자동취소 안내 포함). 같은 날 이전 변경: (**어드민 웹푸시 알림 신설 — 외부 행사 신청이 들어오면 관리자 기기로 푸시**). 계정 없는 비번 어드민 구조 유지: 구독은 계정이 아니라 **기기(브라우저) 단위**. ① 마이그레이션 `2026-09-06_push_subscriptions.sql` — `push_subscriptions`(endpoint UNIQUE·p256dh·auth·label, RLS 0정책 default-deny) 신설, 라이브 적용. ② **`lib/push.ts`** `sendPushToAdmins(payload)` — `web-push`(신규 의존성)로 저장된 구독 전체에 발송, VAPID 키(env) 미설정 시 조용히 skip(solapi와 동일 정책), 404/410 만료 구독 자동 삭제. ③ **`public/sw.js`** 서비스워커 — 푸시 수신 → OS 알림, 클릭 → `/admin/dashboard`. ④ **`POST/DELETE /api/admin/push-subscribe`** — admin_session 쿠키 검증 후 구독 upsert/삭제. ⑤ `/api/event-reg` 신청 성공 후 `sendPushToAdmins` 호출(비차단 try/catch, "🔔 새 행사 신청 — OOO님이 「행사명」에 크루/게스트로 신청"). ⑥ **`PushToggle`**(components/dashboard/PushToggle.tsx) 「🔔 신청 알림 (이 기기)」 ON/OFF — **설정 모달(`EventAlimtalkSettings`) 하단 고정 영역**에 노출(행사 무관한 기기 설정이라 탭 밖 border-t 푸터, 처음엔 헤더 버튼이었다가 같은 날 설정 안으로 이동). 켜면 알림 권한 요청 → sw 등록 → 구독 → 서버 저장(끄면 역순). 미지원 브라우저(iOS 사파리 홈화면 미설치 등)에선 토글 대신 홈 화면 추가 안내 문구 표시. ⑦ env: `NEXT_PUBLIC_VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` — `.env.local` + Vercel Production 등록 완료(프리뷰는 구버전 CLI 이슈로 미등록). 관리자 4명은 각자 기기에서 로그인 후 버튼 1번 누르면 됨. **아이폰은 홈 화면에 추가(PWA)한 상태에서만 푸시 수신**(manifest.json 기존재). 검증: 빌드 통과 + 로컬 dev로 sw.js 200·미인증 307 차단·구독 등록/해제 왕복·테스트 행 정리 확인. 이전 변경: 2026-08-03 (**크루/게스트 구분을 「신청 시점 역할」 기준으로 통일 — 당일 크루 전환해도 집계·보증금 안 바뀜**). 두 갈래 수정, DB 데이터는 무수정. ① **`/api/apply`의 event_registrations 재배정 제거** — 게스트가 크루 지원하면 같은 phone 게스트의 모든 `event_registrations`를 `crew_id`로 갈아치우고 `guest_id=null` 처리하던 로직(flip) 삭제. 앞으로 각 행사 신청은 신청 당시 역할(crew_id XOR guest_id)로 영구 고정. feedbacks 재배정은 유지(익명 집계라 무관). ② **신설 `lib/registration-role.ts`의 `isCrewAtRegistration(crewId, crewCreatedAt, registeredAt)`** — crew_id가 있어도 **크루 가입일(`crew_members.created_at`) > 행사 신청일(`registered_at`)이면 "신청 당시 게스트"로 판정**(크루 폼은 이미 크루만 통과하므로 가입일이 신청일보다 늦으면 게스트 신청이었던 것). 과거에 이미 flip된 행(8/1 행사 당일 크루 전환 7명: 김성인·이주헌·이가인·송영현·조은서·정다현·김정민, 전원 보증금 입금+출석완료)도 **DB 복구 없이 코드 판정만으로 게스트로 복귀**. 적용 3곳: `full-stats`(분석 탭, attendees + thisEventPeople 2지점 — select에 `registered_at`·crew `created_at` 추가), `dashboard-data`(체크인/팀배정/보증금 탭 — 동일 select 추가), `members-list`(신청자 탭 크루/게스트 배지). 검증: 8/1 행사 구 판정 crew 51/guest 16 → **신 판정 crew 44/guest 23**, flip 7명 보증금 탭 복귀(환불 추적 가능). 전환율(`crew_conversion_count`)은 phone+크루 created_at 기준 독립 집계라 영향 없음. 참고: 이은채(크루 가입 4/11 < 신청 7/12, 환불계좌 보유·노쇼확정)는 날짜 기준상 크루로 분류됨(수동 정정 흔적으로 보이는 특이 케이스, 노쇼라 환불 비대상). 이전 변경: 2026-07-22 (**신청자 탭 모바일 검색창 추가**). 신청자(멤버) 탭 검색 input이 `hidden md:block`이라 모바일에선 검색이 아예 없어 명단 조회가 불편했음 → `app/admin/dashboard/page.tsx` 모드토글/정렬 툴바 아래에 **모바일 전용(`md:hidden`) 전폭 검색창 + × 초기화 버튼** 추가(같은 `memberSearch` state 공유, 데스크톱 인라인 검색창은 그대로). 이전 변경: 2026-07-22 (**분석 탭 세그먼트 명단 팝업에 나이 표시**). 차트 클릭 → 세그먼트 명단 팝업에서 학년·전공만 나오고 나이가 없어 나이도 노출. ① `app/api/admin/full-stats/route.ts` **행사 모드** `section1.people`에 `age` 추가 — registrations select의 crew_members/guests에 `age` 컬럼 추가 + attendees·people 매핑에 `age` 포함(crew-all 모드는 이미 age 있었음). ② `app/admin/dashboard/page.tsx` 팝업 카드 메타 줄을 `{grade} · {age}세 · {major}`로 변경(나이 있을 때만). 이전 변경: 2026-07-20 (**현장 출석체크 결과 모달에 크루/게스트 구분 배지**). 게스트만 보증금(5,000원) 대상이라 운영진이 현장에서 즉시 안내할 수 있게, `/checkin` 출석 완료·이미출석 모달의 이름 **위**에 배지 노출: 크루=하늘색 「크루」, 게스트=주황색 「게스트」 + "보증금 5,000원 환불 대상 — 운영진 안내 필요" 한 줄. 구현: `app/api/checkin/route.ts`의 성공 2건·409 3건 응답에 `is_crew: !!crewId` 추가(walk-in/일반 경로 모두), `app/checkin/page.tsx`에 `checkedInIsCrew` state + `TypeBadge` 컴포넌트(값 null이면 미노출). 테스트 단축키: 일반 폼 `테스트`=크루, walk-in 폼 `테스트`=게스트. 이전 변경: 2026-07-09 (**게스트 폼 크루 번호 검증 추가(이중 등록 방지) + 권미진 데이터 정정**). 크루가 게스트 폼으로 신청하면 phone 검증 없이 중복 게스트가 생기던 갭 보완: `/api/event-reg` guest 모드가 신청 전 `crew_members`에서 phone 조회 → 있으면 409 `code:'crew_exists'` 반환하고 게스트 폼이 "크루로 등록된 번호예요" 모달로 크루 폼 안내. 기존 이중 등록 1건(크루 권미진 #256, 010-2442-9837 — 8/1 게스트 신청)은 크루 신청으로 정정 + 중복 게스트 레코드 삭제(라이브 DB). 이전 변경: 2026-07-09 (**내부/외부 행사 토글(설정 창) + 어드민 첫 진입 기본선택 외부행사(8/1) + 신청자 탭 동석자 표시**). events가 공개 행사와 내부 프로젝트 회차를 함께 담게 되면서, 이미 만든 `is_public` 플래그를 **운영진이 설정에서 직접 켜고 끌 수 있게** 노출하고 어드민 기본 선택도 외부행사로. ① **설정 「행사 정보」 탭 상단에 「외부 행사 / 내부 행사」 토글**(`EventAlimtalkSettings` `isPublic` state → `event-settings` POST `is_public` 저장, GET 반환). 외부=신청폼 연결·자동 노출, 내부=출석체크 전용(신청/모니터링 불필요) — 내부 선택 시 알림톡 준비 배너가 "🔒 내부 행사 — 알림톡·신청 불필요"로 전환. ② **어드민 대시보드 첫 진입 기본 선택 = 가장 가까운 외부(공개) 행사** — `/api/admin/events` GET이 `defaultEventId`(=`getActivePublicEventId()`, 없으면 activeEventId) 반환, `loadEvents`가 이걸 초기 선택으로 사용 → 7/18 내부 경쟁PT 대신 **8/1이 뜸**(activeEventId는 날짜기반 그대로 유지, 자동문자 토글용). 드롭다운 옵션은 내부 행사 앞에 `[내부]` 표시(EventItem에 `is_public` 추가). ③ **신청자 탭에 동석자(companion) 표시** — `members-list`가 행사모드에서 `companion` 반환, 모바일 상세카드 + 데스크톱 테이블(팀 옆 「동석자」 열)에 「🤝 이름」 노출(이 페이지 라벨은 '동석자'). 체크인은 종전대로 날짜 기반 `getActiveEventId()`라 내부 행사도 당일 현장 체크인 정상. 이전 변경: 2026-07-09 (**행사 사전신청 동석자 이름 필드 추가(크루·게스트 공통) + 팀 배정 참고 노출**). 행사가 팀플로 진행되는 경우가 많아 신청 시 **같이 오는 동석자 이름을 선택 입력**받아 팀 배정 참고. ① 마이그레이션 `2026-07-09_registration_companion.sql` — `event_registrations.companion TEXT`(nullable) 신설(라이브 적용). ② `/api/event-reg` POST가 crew/guest 공통으로 `companion` 저장(trim, 빈 값 null). ③ 두 폼(`/event-reg/crew`·`/event-reg/guest`)에 「같은 팀 하고 싶은 분(선택)」 Input + 하늘색 고지 박스(제목 "👥 같은 팀으로 배치해 드려요" + 3줄 불릿: 최대한 같은 팀·구성상 다른 팀 가능·**행사 당일 팀 변경 불가** — 현장 팀 변경 요청 방지 목적). ④ 어드민 노출: `dashboard-data`가 `companion` 반환 + `PersonCard`(Attendee)에 「🤝 팀희망 OOO」 보라 배지 → **팀배정·체크인 탭에서 운영진이 참고**(무조건 같은 팀 보장은 아님). ⑤ **8월 행사명 변경 "여름방학 AI 영상 수익화 클래스"로 전체 통일** — events 라이브 title(377a011e)·program_sessions W6 label(라이브+시드)·홈 `CurriculumSection`/`HeroSection` 8월 카드·`app/seminar` 08 카드 전부 반영. 세미나 페이지 06 이모티콘 종료 처리, **08을 「모집 중」으로 바꿔 사전신청 버튼(→ /event-reg = 8/1) 연결**(종전 06에 붙어 있던 버튼 이동). 크루 지원폼 관심프로젝트 옵션(`lib/constants.ts` PROJECTS)의 'AI 영상·음악 콘텐츠 제작'은 행사명 아님 → 유지. 이전 변경: 2026-07-08 (**공개 행사 플래그(events.is_public) — 행사 신청만 필터, 체크인은 날짜 기반 유지 + 수료관리 남은결석 모수 정정**). ① **`events.is_public BOOLEAN NOT NULL DEFAULT true`** 신설(마이그레이션 `2026-07-08_event_is_public.sql`, 라이브 적용). events가 공개 행사와 하반기 프로젝트 내부 회차(경쟁 PT·매칭데이·수료식 등, `program_sessions.event_id` 연동)를 함께 담게 되면서 웹 사전신청이 가장 가까운 미래 행사(7/18 내부 경쟁 PT)로 잘못 연결되던 문제. **`lib/get-active-event.ts`를 공통 `resolveActiveEventId(publicOnly)`로 리팩터** 후 2갈래: `getActiveEventId()`(날짜 기반·모든 행사, **종전 그대로** — 체크인/피드백/어드민이 씀, 내부 회차도 날짜 되면 현장 체크인·출석 정상 동작) / **`getActivePublicEventId()`(is_public=true만, 신규)** — `/api/event-reg`만 사용해 신청을 공개 행사로만 받음. 내부 회차 5건(7/18·8/21·8/28·9/19·11/7)을 false로, **8/1 "AI로 영상·음악 콘텐츠 제작"만 true** → 웹 사전신청이 8/1로 연결(체크인은 날짜대로 7/18 등도 가능). 어드민 드롭다운엔 false 행사도 계속 노출, 「행사 생성」 신규 행사는 기본 true. ② **수료관리 「남은 결석」 모수 정정** — roster API(`app/api/admin/program/roster/route.ts`)와 출석 매트릭스 탭(`app/admin/program/page.tsx` renderRow)이 결석 모수에 아직 안 열린 예정·날짜미정 회차까지 포함해 `absencesUsed = 전체13 − 출석`으로 계산 → 모두 남은결석 0으로 오표기. **오늘(KST) 이전에 열린 회차(`session_date != null && session_date <= today`)만 모수에 포함**하도록 양쪽 수정. ③ **1기 프로그램 팀모임 주차 마감일 세팅(DB 데이터, 코드 변경 없음)** — W2~W12 팀모임/범위 주차가 `session_date=null`이라 ②의 로직에서 모수 제외돼(특히 W2는 출석 33명 기록됐는데 미집계) 회차별 마감일 부여: W2 7/5(추정)·W3 7/12·W5 7/26·W7 8/9·W8 8/16·W11 9/6·W12 9/13(범위·`~날짜` 주차는 그 주가 끝나는 날 = 마감일로 넣어 주가 완전히 지나야 결석 모수 편입). 날짜 있던 클래스/PT 주차(W1 6/27·W4 7/18·W6 8/1·W9 8/21·W10 8/28·W13 9/19)는 유지. 회차 날짜는 `/admin/program` 회차 편집(sessions PATCH)으로 운영진이 직접 수정 가능. 이전 변경: 2026-06-25 (**분석 탭 차트 드릴다운(세그먼트 명단 팝업)**). 분석 탭의 학교/학년/경로/성별 차트는 카운트만 보여줘서 "충남대 7명"이 누군지 알려면 신청자 탭을 다시 봐야 했음 → **막대/파이 클릭 시 해당 카운트의 실제 인원 명단 팝업**(학교별·학년순 정렬, 이름·🍇·성별·학년·전공·연락처 + 행사모드는 출석상태 배지). 구현: ① `app/api/admin/full-stats/route.ts`의 `section1`에 `people` 배열 추가(카운트와 동일 모집단 — 행사모드=포도 제외 참가자 with `is_crew`, crew-all모드=전체 크루 with `is_member`). 행사모드 registrations select에 `name, major` 추가. ② `app/admin/dashboard/page.tsx`: `MiniBarChart`/`MiniPieChart`에 `onPick` 콜백(Bar/Pie onClick), `SegPerson` 타입, `segment` 상태 + `openSeg(dim, label, target, value)` 헬퍼(target: all/crew/guest/saeng로 모집단 필터 후 dim===value 매칭, 학교→학년(`gradeRank`)→이름 정렬), 차트 config에 `dim` 키, 인라인 팝업(학교 바뀔 때 소제목 헤더). 데스크톱 차트별 target(전체=all/크루=crew/게스트=guest/비포도=saeng), 모바일은 `chartTarget` 사용. **이전 변경: 2026-06-25 (MCP 설정 + 데드 코드 정리)**. ① 프로젝트 루트 `.mcp.json` 신설(git·supabase·github·vercel MCP 서버, claude.ai 통합과 무관하게 Claude Code CLI/다른 MCP 클라이언트에서 동작. 토큰은 셸 환경변수 `${SUPABASE_ACCESS_TOKEN}`·`${GITHUB_PAT}`로 참조 — 비밀값 미커밋. supabase는 `--project-ref=xtbtufdycegfylsoerqq` 스코프. 설정 가이드 `docs/mcp-setup.md`). ② **데드 코드/찌꺼기 제거**(`npm run build` 통과 검증) — 대시보드 페이지가 recharts를 인라인으로 그려 import 안 하던 차트 컴포넌트 9개(`StatCard`/`SchoolChart`/`GradeChart`/`PathChart`/`DateChart`/`FunnelChart`/`FeedbackRadar`/`FeedbackTagChart`/`MembersTable`) + 미사용 `components/feedback/StepIndicator.tsx` 삭제(이제 `components/dashboard/`엔 `EventAlimtalkSettings.tsx`만 남음). 1회용 `fix_metadata.sh`·빈 `content/` 폴더·`.DS_Store`들 삭제. 전 코드에서 사용 0회인 `react-icons` 의존성 제거(`npm uninstall`). 이전 변경: 2026-06-01 (**기수(cohort) + 하반기 프로젝트(수료 관리) 서브시스템 신설** — 행사 독립 운영을 깨지 않고 더하는 additive 변경). 기존 시스템은 행사가 서로 독립이라 여러 회차 누적 출석·수료 판정을 담을 수 없었음. 하반기 13주 프로젝트에 ~30명 크루가 등록해 **개인 출석 3/4(결석 4회까지) + 팀 결과물 기한내 제출 → 수료증** 기준을 관리해야 해서 신규 서브시스템 구축. 청강·게스트는 기존 행사/게스트 모델 유지(수료 무관). 상세는 **14절**. 요약: ① 마이그레이션 `2026-06-01_cohort_program.sql` — `cohorts`(기수) + `events.cohort_id`(1기 백필) + `programs`(룰 설정) + `program_sessions`(13주+OT) + `program_teams` + `program_enrollments`(명단) + `program_attendance`(매트릭스 셀) + `program_projects`/`program_deliverable_specs`/`program_deliverables`(결과물) 9개 테이블, 전부 BIGINT PK·크루 FK BIGINT·RLS 0정책 default-deny. 라이브 DB 적용 완료(1기·1프로그램 시드, events 7건 백필). ② 순수 룰엔진 `lib/program-eligibility.ts`(결석 4 초과→수료취소, 주당1회 cap, 출석 AND 결과물). ③ 체크인 브릿지 `lib/program-checkin-bridge.ts` — 공식행사(`session.event_id` 연동)에 등록 크루가 `/checkin` 시 자동 출석(게스트/청강·미연동 무영향). `app/api/checkin/route.ts` 출석 확정 3지점에 `bridgeProgramAttendance` 1줄씩. ④ 어드민 페이지 `/admin/program`(4탭: 참가자 명단/출석 매트릭스/결과물/팀 관리) + 신규 라우트 `/api/admin/cohorts`·`/api/admin/cohorts/set-current`·`/api/admin/program`(GET/PATCH)·`/program/roster`·`/enroll`·`/enrollment-status`·`/sessions`·`/attendance`·`/attendance-toggle`·`/projects`·`/deliverables`·`/deliverable-toggle`·`/teams`·`/assign-team`. ⑤ 기수 레트로핏: `lib/get-active-cohort.ts`(`getCurrentCohortId`), `/api/admin/events` GET이 `?cohort_id` 필터(없으면 현재 기수, `all`=전체)+`cohorts`/`currentCohortId` 반환·POST는 현재 기수 자동 부착, 대시보드 헤더에 기수 드롭다운(기수 2개+ 시 노출)+「수료관리」링크. ⑥ 1기 13주 일정·프로젝트 3종은 `/admin/program`의 「1기 기본 일정 생성」 버튼 또는 시드로 구성(현재 라이브에 15세션·3프로젝트·5산출물 적재됨). 엔드투엔드 검증 완료(룰엔진 경계값 9/10/8주, 체크인 브릿지 source='checkin', 테스트 데이터 정리). 이전 변경: 2026-06-01 (문서·DB 정합성 점검 — 코드 변경 없음). 라이브 Supabase(`xtbtufdycegfylsoerqq`) 전수 대조 결과 API·알림톡 템플릿·페이지·컴포넌트·출석탭 로직은 코드와 완전 일치. DB 문서 정정 3건: ① 4-3절 — guests의 `project`/`motivation` 제거는 마이그레이션 파일이 아니라 운영 DB에서 **직접** 제거된 것으로 문구 정정(`supabase/migrations/`에 `remove_motivation_project_from_guests.sql` 없음 확인). ② 4-4절 — `event_registrations.crew_id` 타입을 UUID → **BIGINT**로 정정(crew_members.id가 BIGINT라 FK도 BIGINT). ③ `supabase/schema.sql` 을 라이브 DB 기준으로 **전면 재생성**(구버전 스냅샷은 crew_members.id를 UUID로, feedbacks에 score/crew_id/guest_id 존재로, events 5컬럼·auto_checkin·reports·alimtalk_logs 누락, RLS allow-all 정책 등 현실과 어긋났음 → 7개 테이블·실제 컬럼·인덱스·RLS 0정책 default-deny 반영). migrations 폴더가 단일 출처이고 schema.sql은 스냅샷임을 파일 상단에 명시. 이전 변경: 2026-05-20 (출석 탭 알림톡 발송 방식 개편 — 상태 변경은 문자 없이 조용히, 문자는 개별 버튼으로 수동, 현장 체크인 자동문자는 행사별 토글). ① `update-status` 라우트에서 알림톡 자동 발송 **완전 제거**(노쇼확정 전환 시 9·10번 보내던 로직 삭제) — 출석/노쇼/미출석 전환은 이제 문자 없이 처리(사전 불참 통보자도 부담 없이 체크/해제). 단 크루 `noshow_count` 증감은 유지(데이터 정합성). 대시보드 `handleUpdateStatus`의 `window.confirm`·알림톡 토스트도 제거. ② **새 라우트 `POST /api/admin/send-individual-alimtalk`** — 개별 수동 발송. `{registration_id, type:'checkin'|'noshow'}`. `checkin`→6/7번(팀 유무 분기, 크루·게스트 공통), `noshow`→9번(+누적 2회면 10번, 크루 한정). 출석 탭 컬럼별 버튼에서 호출. ③ 출석 탭 UI: **출석완료 컬럼** 카드마다 「출석문자」 버튼(checkin 발송) + 「미출석」, **노쇼확정 컬럼** 카드마다 「노쇼경고」 버튼(crew만 노출) + 「해제」. ④ **현장 체크인(/checkin) 자동 출석문자 토글** — `events.auto_checkin_alimtalk BOOLEAN DEFAULT false`(마이그레이션 `2026-05-20_auto_checkin_alimtalk.sql`) 신설. `/checkin`의 `sendCheckinAlimtalk`가 발송 전 활성 행사의 이 플래그를 조회해 **true일 때만 자동 발송**(기본 OFF=발송 스킵). 출석 탭 「출석완료」 헤더에 `자동문자 ON/OFF` 토글 버튼 — **활성 행사(`selectedEventId === activeEventId`) 선택 시에만 노출**(현장 체크인은 활성 행사만 처리하므로). **새 라우트 `POST /api/admin/toggle-auto-checkin`**(`{eventId, enabled}`)로 저장. `/api/admin/events` GET이 `auto_checkin_alimtalk` 반환 + 대시보드가 `activeEventId` state 보관. 이전 변경: 2026-05-21 (events DB만 수정: 2026-05-30 행사명 `PROGEN 1기 OT - 청년마을 만들기 협업 프로젝트` → `'청년마을 만들기' X PROGEN 실전 프로젝트 OT`, 시작시각 14:00 → 15:00 KST(event_date=`2026-05-30 06:00:00+00`). 행사는 15:00~16:30 진행이나 events 테이블에 종료시간 컬럼 없어 시작시각만 반영. 코드 변경 없음. 이전 변경: 2026-05-15 ① 신청자 탭 새로고침 버튼을 「+ 크루 추가」로 교체(상단 헤더에 새로고침 별도 존재) + 새 라우트 `POST /api/admin/add-registration`(eventId·crewId → event_registrations에 사전신청 INSERT + 행사정보 ready 시 2번 크루용 확정 알림톡 자동 발송) + `app/admin/dashboard/page.tsx`에 `AddCrewModal` 신설(크루 명단 검색·선택·추가, 이미 신청된 크루 제외, 이름 가나다순). 행사 신청자 모드(`membersMode === 'event'`)에서만 노출, crew-all 모드 제외. ② 어드민 "설정" → "알림톡 발송" 탭 미발송자 명단 가나다 정렬(이름 빨리 찾기 위함, `event-settings` GET에서 `localeCompare(name, 'ko')`로 정렬). ③ 어드민 "설정" → "알림톡 발송" 탭에 미발송자 체크박스 명단 추가 — 참석확정(2번)·D-1 공지(4번)에 대해 미발송 대상 명단을 표시하고 운영진이 일부만 골라 발송 가능. `GET /api/admin/event-settings`가 `recipients.confirm/d1` (각 `{id, name, type: 'crew'|'guest', sent}`) 반환 + confirm sent 카운트가 크루용(`EVENT_CONFIRMED_CREW.code`)/게스트용(`EVENT_CONFIRMED.code`) 템플릿 분리되어 정확히 집계되도록 수정. `POST /api/admin/send-alimtalk-batch`에 선택적 `registrationIds: string[]` 필터 옵션 — 있으면 그 ID만, 비우면 기존처럼 미발송자 전원. `components/dashboard/EventAlimtalkSettings.tsx`에 `RecipientChecklist` 추가(미발송자 체크박스, 발송완료자 회색 line-through, 크루/게스트 배지). 일정/장소 변경(8번)은 그대로 전원 발송. 이전 변경: 2026-05-13 참석 확정(2번) 알림톡을 크루용/게스트용 2종으로 분리 — `lib/solapi.ts`의 `ALIMTALK`에 `EVENT_CONFIRMED_CREW`(`KA01TP260512222214758QYG80poi9FS`, 보증금 문구 없음) 신설 + 기존 `EVENT_CONFIRMED`는 게스트 전용으로 라벨 변경. `/api/event-reg` mode=crew는 새 크루용으로, `/api/admin/toggle-deposit`은 그대로 게스트용. `/api/admin/send-alimtalk-batch` template=confirm은 대상이 크루(crew_id 존재)면 크루용, 아니면 게스트용으로 row별 분기 발송. dedup도 template_code 기준이라 자연스럽게 분리됨. → 6번 표 / 이전 변경: 2026-05-11 ① 카카오 알림톡(솔라피) 시스템: 템플릿 10종 + lib/solapi.ts + events 행사정보 5컬럼 + alimtalk_logs 테이블 + 어드민 "설정" 모달 + 보증금 탭 「취소 알림」 버튼 + 7개 라우트 발송 연결. ② 3·4·5월 행사명 변경(events 테이블 + 홈/세미나/아카이브), 2026-05-30 "PROGEN 1기 OT" events row 추가. ③ 팀배정 탭 정렬 버튼. ④ 모바일 가로 스크롤·blur blob 수정. ⑤ 데드 코드 정리: StarField/AboutSection/NumbersSection/ActivitiesSection/StarRating/getLatestEventId/dashboard·full/SCORE_LABELS/supabase-browser.ts 삭제 + next-mdx-remote·gray-matter 의존성 제거 + 잔존 파일(standalone HTML, 크루명단 xlsx) 삭제. ⑥ 알림톡 `#{프로그램명}` 변수를 `lib/solapi.ts`의 `programLabel()`로 낫표(`「행사명」`)로 감싸 행사명 강조 — solapi.ts의 `vars*` 5종 + send-cancel-alimtalk/update-status/send-alimtalk-batch 라우트 모두 적용. ⑦ `lib/solapi.ts`의 `ALIMTALK` 맵 templateId 10종을 솔라피 실제 ID(`KA01TP260511...`)로 교체(이전엔 placeholder). 솔라피 채널 `KA01PF260511054914846rCGGdEqH9tS`(searchId `progen`). 10종 중 #5(신청 취소 확인)만 검수 승인(APPROVED), 나머지 9종은 검수중(INSPECTING) — ID 맞아도 검수 통과 전엔 발송 안 됨. → 6번·12번 표 참고. ⑧ 홈 `CurriculumSection` 카드 제목 레이아웃 변경: `headline`(키워드)을 작은 폰트(`text-[11px] md:text-xs`)·sky-500 라벨로 제목 윗줄에 분리, `title`(행사명)은 큰 굵은 글씨(`text-lg md:text-2xl font-black`)로 강조. 종료 회차는 키워드 `#aaa`/제목 `#888`.)
...
| program_name_text | TEXT (nullable) | **알림톡 `#{프로그램명}` 표기 오버라이드** — 입력돼 있으면 이 텍스트, 비어 있으면 `title` 그대로(`lib/solapi.ts` `programNameOf(ev)`). 설정 「행사 정보」 탭 「알림톡 행사명 표기 (선택)」 입력칸으로 편집. 전 템플릿의 프로그램명 발송 경로 10곳 적용(체크인 6·7, 개별발송, 취소 5, 미입금 11, sms-hook 입금완료, batch, 테스트 포함). 마이그레이션 `2026-09-15_event_program_name_text.sql` |
| datetime_text | TEXT (nullable) | **알림톡 `#{일시}` 표기 오버라이드** — 입력돼 있으면 이 텍스트 그대로(예: "9월 19일(토) 오후 2시~5시"), 비어 있으면 `event_date`를 `formatEventDateKo()`로 자동 포맷(`lib/solapi.ts` `eventDateLabel()`). 설정 「행사 정보」 탭 「알림톡 일시 표기 (선택)」 입력칸으로 편집. 1번 신청접수·2번 크루확정·4번 D-1이 사용. 마이그레이션 `2026-09-08_event_datetime_text.sql` |
...
| is_event | BOOLEAN (NOT NULL, default false) | **이벤트(프로모션성) 구분** — true면 `/admin/event` 전용 어드민에서만 집계되고 일반 행사 어드민(`/api/admin/events` 기본 필터)·웹 행사 신청에서 제외. 신청은 전용 폼 **`/event`**(게스트 폼에서 보증금 안내·환불계좌 제거, `kind:'event'` 제출 → `getActivePromoEventId()`가 is_event=true 중 날짜 기반 선택). 이벤트 신청은 **알림톡 미발송**(접수 템플릿에 보증금 문구 포함), 크루 번호 차단(crew_exists) 스킵(단일 폼), 관리자 푸시는 「새 이벤트 신청」으로 `/admin/event` 링크. 이벤트 row는 is_event=true + **is_public=false**로 생성(POST /api/admin/events에 `is_event:true` 전달 시 자동). 어드민 UI는 `app/admin/dashboard/Dashboard.tsx`(공용 컴포넌트, `kind` prop) — `/admin/dashboard`=regular, **`/admin/event`=event**(기수 셀렉터·크루 전체 옵션 숨김, 헤더 「이벤트」 뱃지 + 행사↔이벤트 어드민 전환 링크). 마이그레이션 `2026-09-15_event_is_event.sql` |
| is_public | BOOLEAN (NOT NULL, default true) | **외부(공개)/내부 행사 구분.** 어드민 "설정" → "행사 정보" 탭 상단 **「외부 행사 / 내부 행사」 토글**로 운영진이 직접 변경(`event-settings` GET/POST가 `is_public` 처리). **외부**=웹 사전신청 연결 O·자동 선택 대상. **내부**=신청/모니터링 불필요, 행사 당일 현장 체크인 전용. **`getActivePublicEventId()`(신청 전용 + 어드민 첫 진입 기본 선택)가 is_public=true만 필터** — 내부 회차(경쟁 PT·매칭데이·수료식 등)로 신청/기본선택이 가지 않게. **체크인(`/checkin`)·피드백은 종전대로 날짜 기반 `getActiveEventId()`(모든 행사)** — 내부 회차도 날짜가 되면 현장 체크인/출석 정상 동작. 어드민 드롭다운엔 false 행사도 계속 노출(제목 앞 `[내부]` 표시). 신규 행사(어드민 「행사 생성」)는 기본 true. 마이그레이션 `2026-07-08_event_is_public.sql`. 현재 false: 7/18 경쟁PT·8/21 매칭데이·8/28 웹교육·9/19 최종PT·11/7 수료식 (프로그램 내부 회차, `program_sessions.event_id` 연동) |
...
| student_number | TEXT (nullable) | 학번. 크루 지원 폼엔 입력란 없음 — **행사 사전신청 시 입력한 학번을 `/api/event-reg`가 크루 레코드에도 저장**(매 신청마다 최신 값 갱신). 기존 신청분 26명 백필(2026-09-17). 마이그레이션 `2026-09-17_crew_student_number.sql` |
...
### 4-5b. grape_feedbacks (포도용 피드백, 기명 — 2026-09-21 신설)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | UUID (PK) | 자동 생성 |
| event_id | UUID (FK → events, ON DELETE SET NULL, nullable) | 제출 시점 활성 행사(`getActiveEventId`) |
| name | TEXT (NOT NULL) | 작성자 이름 (기명) |
| q1 | TEXT | 생명과 친해질 수 있었나요? 어떤 부분이 도움이 되었나요? |
| q2 | TEXT | 애로사항이 있었나요? |
| q3 | TEXT | 생명을 2차 만남 혹은 이후 계획이 있으신가요? |
| created_at | TIMESTAMPTZ | 제출일 |

> `/grapefeedback` 폼 전용. 일반 feedbacks(익명)와 달리 **기명**이고 질문 구조가 다름. RLS 활성화 + 정책 0개(default-deny). 분석 탭 「🍇 포도용 피드백」 카드(full-stats `section3.grape_responses`)에서 열람. 마이그레이션: `2026-09-21_grape_feedbacks.sql`.
...
### 4-8. push_subscriptions (어드민 웹푸시 구독, 2026-09-06 신설)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | UUID (PK) | 자동 생성 |
| endpoint | TEXT (NOT NULL, UNIQUE) | 푸시 서비스 endpoint URL — 기기 식별자 역할 (upsert 키) |
| p256dh | TEXT (NOT NULL) | 구독 암호화 공개키 |
| auth | TEXT (NOT NULL) | 구독 인증 시크릿 |
| label | TEXT (nullable) | 기기 메모 (선택, 현재 UI에선 미입력) |
| created_at | TIMESTAMPTZ | 등록일 |

> **계정 없는 어드민 구조라 구독은 "기기(브라우저)" 단위** — 관리자가 대시보드 「🔔 알림」 토글을 누른 기기가 1행. 외부 행사 신청 시 `lib/push.ts` `sendPushToAdmins()`가 전체 행에 발송, 404/410(만료·해지) 구독은 자동 삭제. RLS 활성화 + 정책 0개(anon default-deny). 마이그레이션: `2026-09-06_push_subscriptions.sql`.
...
4. **게스트→크루 전환**: 같은 phone의 guests가 있으면 그 게스트의 `feedbacks`만 새 crew_id로 마이그레이션. **`event_registrations`는 재배정하지 않음**(2026-08-03) — 각 행사 신청은 신청 시점 역할(게스트/크루)로 고정해 분석 탭 참여율·게스트 보증금 추적을 보존. 어드민 화면의 크루/게스트 판정은 `lib/registration-role.ts` `isCrewAtRegistration()`(크루 가입일 vs 신청일 비교)이 담당해 과거 flip된 행도 게스트로 표시. 전환율은 `full-stats`의 `crew_conversion_count`가 독립 집계
...
- 게스트: **먼저 phone이 `crew_members`에 있으면 409 `code:'crew_exists'` 반환 → 게스트 폼이 "크루로 등록된 번호예요" 모달로 크루 폼(`/event-reg/crew`) 안내**(2026-07-09 신설, 게스트/크루 이중 등록 방지). 아니면 `guests`에 phone 기준 upsert (신규면 source_event_id 설정)
...
6. 응답에 `team_name` + **`is_crew`** 포함 → "OOO님 출석 완료, X팀입니다" Modal. **모달 이름 위에 크루(하늘)/게스트(주황) 배지** — 게스트면 "보증금 5,000원 환불 대상 — 운영진 안내 필요" 문구 동반(2026-07-20). '이미 출석' 모달도 동일 배지
...
### 5-6b. 포도용 피드백 `/grapefeedback` ([app/grapefeedback/page.tsx](app/grapefeedback/page.tsx), 2026-09-21 신설)
- **한 화면 단일 폼**(스텝 없음): 이름(필수) + 3문항 textarea(전부 필수) + 제출 버튼.
  1. 생명과 친해질 수 있었나요? 어떤 부분이 도움이 되었나요?
  2. 애로사항이 있었나요?
  3. 생명을 2차 만남 혹은 이후 계획이 있으신가요?
- 처리: `POST /api/grape-feedback` → `grape_feedbacks`에 귀속 행사(`getFeedbackEventId` — 행사 후 7일까지 직전 행사 우선) 연결 INSERT. 성공 시 감사 화면.
- 열람: 어드민 분석 탭 「피드백 원문」 아래 **「🍇 포도용 피드백」 카드**(이름 + 3문답, violet 톤).
...
- 새로고침 버튼
- **설정 버튼** (실제 행사 선택 시에만 노출, 'crew-all' 모드 제외) → `EventAlimtalkSettings` 모달: 탭 A "행사 정보"(**상단 「외부 행사 / 내부 행사」 토글**=is_public, 내부면 알림톡/신청 불필요 안내로 전환 + 장소·입장시간·준비물·당일진행·채팅방링크 입력·저장 `/api/admin/event-settings`) / 탭 B "알림톡 발송"(참석확정 2번 미발송자 일괄발송 / D-1 공지 4번 발송 / 일정·장소 변경 8번 발송 — 모두 `window.confirm` 후 `/api/admin/send-alimtalk-batch`)
- 로그아웃 버튼

> **🔔 신청 알림 토글** (`components/dashboard/PushToggle.tsx`, 2026-09-06): 설정 모달 **하단 고정 영역**(탭 밖)에 노출 — 이 기기로 신청 웹푸시 받기 ON/OFF. 켜면 알림 권한 요청 → `/sw.js` 등록 → 푸시 구독 → `/api/admin/push-subscribe` 저장. 푸시 미지원 브라우저에선 토글 대신 아이폰 홈 화면 추가 안내 문구.
...
- **(2026-05-20, 노출조건 2026-09-06 완화) 자동문자 토글**: 「출석완료」 컬럼 헤더에 `자동문자 ON/OFF` 버튼 — **실제 행사 선택 시 항상 노출**(crew-all 모드 제외). 종전엔 활성 행사(`selectedEventId === activeEventId`)에서만 노출했는데, 내부 회차(9/18 최종PT)가 공개 행사(9/19)보다 앞 날짜면 어드민 기본선택(공개 행사)과 날짜기반 활성 행사가 어긋나 토글이 숨어버리는 문제로 완화. 플래그는 행사별(`events.auto_checkin_alimtalk`) 저장이고 발송은 그 행사가 활성(당일)일 때 현장 체크인에서만 일어나므로 미리 켜둬도 안전. ON이면 현장 체크인(/checkin) 시 출석문자 자동 발송, OFF(기본)면 스킵. `toggle-auto-checkin`이 저장.
...
- **팀명 클릭 → 팀 순서 교체 드롭다운(2026-09-19)**: 전체 팀 목록 select가 뜨고 다른 팀 선택 시 두 팀 멤버 전원을 맞바꿈(`handleSwapTeams` — 낙관적 UI 후 멤버별 `/api/admin/assign-team` 호출, 별도 API 없음). 빈 팀 선택 시 그 슬롯으로 이동. 종전 인라인 팀명 rename은 제거됨
...
- 검색 (이름·연락처·계좌번호)
- 행마다: 이름 + 포도 점 + 연락처 / 환불 계좌(없으면 "계좌 미입력" 안내) + 「수정」 → 인라인 입력으로 직접 추가·갱신 (`/api/admin/update-refund-account`) / 상태 뱃지 클릭 → 사이클 (`/api/admin/toggle-deposit`) / **통합 알림 버튼(미입금 행에만 노출, 2026-09-06)** — 총 3회 시퀀스: 「미입금 알림 n/2」(11번, amber) → 2회 소진 시 「취소 알림 (3회차)」(5번, red) → 발송 후 「취소 알림 완료」(회색 텍스트). 클릭하면 커스텀 팝업(Modal)에 발송 현황 + 단계별 발송 버튼 + **「바로 취소 알림 보내기」**(미입금 알림 생략하고 즉시 5번 발송). 라우트: `/api/admin/send-deposit-reminder`(11번, 최대 2회 서버 검증) / `/api/admin/send-cancel-alimtalk`(5번). 횟수·취소여부는 `dashboard-data`의 `deposit_reminder_count`·`cancel_notice_sent`(alimtalk_logs 집계). 11번 템플릿 검수 통과 전엔 미입금 알림 발송 실패 토스트. **실제 신청 행 삭제는 운영진이 신청자/멤버 탭에서 직접** (팝업은 알림톡만 보냄)
...
| POST | `/api/feedback` | 익명 피드백. 귀속 행사는 `getFeedbackEventId()`(행사 후 7일까지 직전 행사 우선, 2026-09-21) |
| POST | `/api/grape-feedback` | 포도용 기명 피드백 (`/grapefeedback` 폼). 이름+3문항 전부 필수, `getFeedbackEventId()` 행사에 연결 INSERT. 2026-09-21 신설 |
| POST | `/api/admin/login` | 관리자 로그인 |
| POST | `/api/admin/logout` | 관리자 로그아웃 |
| POST | `/api/sms-hook` | 보증금 입금 SMS 웹훅(공기계 SmsForwarder 전달). `SMS_HOOK_SECRET` 자체 인증. 입금 5,000원 + 미입금 게스트 이름 매칭 시 자동 입금 처리 + 2번 알림톡 + 관리자 푸시. 미매칭/동명이인은 수동확인 푸시만. `test:true`면 부작용 없음. 2026-09-08 신설 |
...
| GET | `/api/admin/members-list` | 신청자 명단 (상세, mode=all 옵션). 행사모드는 `companion`(동석자) 포함 |
| GET | `/api/admin/export` | CSV 내보내기 (UTF-8 BOM) |
| GET | `/api/admin/dashboard-data` | 대시보드 통합 데이터 |
| GET | `/api/admin/full-stats` | 분석 탭 3개 섹션. `section1.people`(개인 명단: 이름·학교·학년·성별·전공·연락처+`is_crew`/`is_member`/`status`)도 반환 — 차트 클릭 세그먼트 명단 팝업용. `section3.grape_responses`(포도용 피드백 이름+3문답, crew-all 모드는 빈 배열)도 반환(2026-09-21) |
| GET / POST | `/api/admin/events` | 행사 목록(GET은 `auto_checkin_alimtalk`·`is_public` 포함 + `activeEventId`(날짜기반) + `defaultEventId`(외부행사 기준 어드민 기본선택)) / 행사 생성 |
...
| POST | `/api/admin/send-cancel-alimtalk` | 5번(신청 취소 확인) 알림톡 1건 수동 발송 (보증금 탭 통합 알림 팝업 3회차 또는 「바로 취소 알림」 버튼). registration_id만 받음. 행 삭제는 별도 |
| POST | `/api/admin/send-deposit-reminder` | 보증금 미입금 안내(11번) 1건 수동 발송 (보증금 탭 통합 알림 팝업 1·2회차). 게스트+미입금 상태 서버 재검증(아니면 400/409) + **최대 2회 제한**(alimtalk_logs sent 카운트 기준, 초과 시 409 — 3회차는 취소 알림). 성공 응답에 `count` 포함. 2026-09-06 신설 |
...
| POST / DELETE | `/api/admin/push-subscribe` | 어드민 웹푸시 구독 등록(endpoint 기준 upsert)/해제. 대시보드 `PushToggle` 버튼이 호출. admin_session 쿠키 검증(미들웨어 + 라우트 자체 checkAuth). 2026-09-06 신설 |
| POST | `/api/admin/test-alimtalk` | 알림톡 테스트 1건 발송. `{eventId, template: ALIMTALK 키, phone}` — 12종 전 템플릿 지원, 수신자명 '테스트', 행사 변수는 해당 행사 기준(8번은 플레이스홀더 값). 설정 모달 「알림톡 발송」 탭 하단 🧪 테스트 박스(템플릿 드롭다운+번호 입력)가 호출. alimtalk_logs에 기록되나 사람 FK 없이 eventId만 담아 dedup·발송횟수 집계에 영향 없음. 2026-09-08 신설 |
| GET / POST | `/api/admin/event-settings` | 행사 정보(location/entry_time/materials/program_detail/kakao_chat_url) + **`is_public`(내부/외부 토글)** 조회·저장 + 알림톡 미발송자 카운트(confirm/d1) + 명단(`recipients.confirm`/`recipients.d1`: `{id, name, type, sent}`). POST는 `is_public`이 boolean으로 오면 반영 |
...
> `#{프로그램명}` 변수 값은 `programLabel()`이 행사명 그대로 넘김(2026-09-09 낫표 「」 감싸기 제거 — 운영자 요청).

| # | 키 | 템플릿 코드 (솔라피 실제 ID) | 변수 | 발송 시점 |
|---|-----|------------|------|----------|
| 1 | EVENT_REG_RECEIVED | `KA01TP260906103329534uNkFHnMsp0a` ✅승인 (2026-09-08 신규 교체) | 이름·프로그램명·일시 (`varsEventRegReceived`, 장소 변수 삭제됨) | `/api/event-reg` mode=guest 즉시. 본문에 보증금 계좌·3일 자동취소 안내 포함 |
| 2-게스트 | EVENT_CONFIRMED | `KA01TP260906103529205rx0U0oQ90Q7` ✅승인 (2026-09-08 신규 교체) | 이름·프로그램명 + 버튼 `#{url}`(오픈채팅 코드) — `varsEventConfirmedGuest` | `toggle-deposit` 미입금→입금 / **`/api/sms-hook` 자동 입금 확인 시** / `send-alimtalk-batch` template=confirm (게스트). **발송 조건 완화: `confirmChatReady`(채팅방 링크만 있으면 됨 — 종전 5개 필드 요구하던 `eventConfirmReady`·`varsEventConfirmed` 삭제)**. 본문에 보증금 환불 안내 포함 |
| 2-크루 | EVENT_CONFIRMED_CREW | `KA01TP260906112122409DXPjnrJCl2k` ✅승인 (2026-09-08 신규 교체) | 이름·프로그램명·일시 + 버튼 `#{url}` — `varsEventConfirmedCrew` | `/api/event-reg` mode=crew / `add-registration` / `send-alimtalk-batch` template=confirm (크루). 발송 조건 동일하게 `confirmChatReady`. 보증금 문구 없음 |
| 3 | CREW_CONFIRMED | `KA01TP260511070701744h8gIXOphWEW` | 이름 (버튼 고정 링크) | `/api/apply` 성공 즉시 |
| 4 | EVENT_D1_NOTICE | `KA01TP260915043622989u7WTWk06rij` ✅승인 (2026-09-18 「행사 사전 공지(사본)」으로 교체 — **변수 축소: 입장시간·진행내용 삭제**. 구 `KA01TP260907...`은 솔라피에 잔존하나 코드 미사용) | 이름·프로그램명·일시·장소·준비물 + 버튼 `#{url}` | `send-alimtalk-batch` template=d1 + `round`(1|2) (설정 탭 「행사 사전 공지」 1차/2차 토글, 수동). **같은 템플릿 2회 발송 운영(1차=확정 대체, 2차=행사 전날) — 회차별 중복 방지는 alimtalk_logs 발송 횟수(count>=round) 기준이며 template_code 단위라 코드 교체 시 횟수 0부터 재집계**. cron 미구현 |
...
| 11 | DEPOSIT_REMINDER | `KA01TP260906071133136qAave0oFtch` ✅승인 (2026-09-08 확인) | 이름·프로그램명 | `send-deposit-reminder` (보증금 탭 「미입금 알림」 버튼, 미입금 행에만 노출, 수동). 본문: 입금 계좌 + 참석 시 전액 반환 안내. 검수 통과되어 정상 발송 가능 |
...
- **[lib/get-active-event.ts](lib/get-active-event.ts)**: 활성 행사 결정 로직(공통 `resolveActiveEventId(publicOnly)`). **`getActiveEventId()`(날짜 기반, 모든 행사)** — 체크인(/checkin)·피드백. 내부 프로젝트 회차도 날짜가 되면 활성 행사로 잡혀 현장 체크인/출석 정상 동작. **`getActivePublicEventId()`(is_public=true만)** — 웹 행사 사전신청(/event-reg) + **어드민 대시보드 첫 진입 기본 선택**(`/api/admin/events`가 `defaultEventId`로 반환), 내부 회차로 신청/기본선택이 잘못 가는 것 차단(2026-07-08 신설, 2026-07-09 어드민 기본선택에도 적용). **`getFeedbackEventId()`(2026-09-21 신설)** — 피드백 전용 귀속 행사: 당일 포함 가장 최근에 지난 행사가 7일 이내면 그 행사 우선, 없으면 최근접 미래 행사, is_event(프로모션) row 제외 — `/api/feedback`·`/api/grape-feedback` 사용. 둘 다(getActiveEventId/getActivePublicEventId) 오늘 자정 이후 최근접 미래 행사 우선, 없으면 과거 행사 fallback은 **PAST_FALLBACK_DAYS=7일 이내**일 때만. 그 이상 지난 후 다음 행사 row가 없으면 `null` 반환 → API들이 "현재 활성 행사를 찾을 수 없습니다" 안내. 운영진의 다음 달 events row 등록 누락 시 사용자가 지난 행사로 잘못 신청되는 사고 방지 목적.
...
- **[lib/push.ts](lib/push.ts)** (2026-09-06): 어드민 웹푸시 헬퍼. `sendPushToAdmins({title, body, url})` — `web-push` 패키지로 `push_subscriptions` 전체에 발송(VAPID 서명, 연락처 mailto:aipro2510@gmail.com). **env 2종(NEXT_PUBLIC_VAPID_PUBLIC_KEY/VAPID_PRIVATE_KEY) 미설정 시 조용히 skip** — 사이트 동작 안 막음(solapi와 동일 정책). 발송 실패 404/410(만료·해지)은 해당 구독 행 자동 삭제. 현재 호출처: `/api/event-reg` 신청 성공 직후 1곳(비차단 try/catch). 서비스워커는 `public/sw.js`(푸시 표시 + 클릭 시 어드민 열기).

**RLS** (2026-04-26 잠금 적용, 2026-05-11 alimtalk_logs 추가, 2026-09-06 push_subscriptions 추가):
- 8개 테이블 (`crew_members`, `events`, `event_registrations`, `guests`, `feedbacks`, `reports`, `alimtalk_logs`, `push_subscriptions`) 모두 RLS 활성화 + **정책 0개 = anon default-deny**. (14절의 program 계열 9개 테이블도 동일)
...
| 알림톡 발신번호 | **해결(2026-09-06)** | 운영자가 발신번호를 010-4323-2510 → **010-7698-2280**으로 변경(솔라피 등록 발신번호도 새 번호만 active). 옛 번호가 env에 남아 발송 시 **3058 「전송경로 없음」 실패**하던 것을 `.env.local` + Vercel Production `SOLAPI_SENDER_PHONE=01076982280`으로 교체 + 프로덕션 재배포로 해결. 실발송 테스트(5번 템플릿 → 운영자 번호) **수신 완료(4000)** 검증. 참고: 알림톡의 실제 발신 주체는 카카오 채널(pfId)이고 발신번호는 SMS 대체발송·필수 from 필드용 — 카카오비즈니스 채널 프로필의 전화번호 변경은 코드/설정과 무관. 발신번호를 또 바꾸면 ①솔라피 콘솔에 새 번호 등록 ②env 2곳 교체 ③재배포 |
...
| 보증금 자동 입금 확인 | **반자동(2026-09-08, SMS 웹훅)** | **채택안: 하나은행 입출금 문자통지(월 800원) → 안드로이드 공기계 SmsForwarder 앱 → `/api/sms-hook`** — 5,000원 입금+이름 매칭 시 자동 입금 처리, 미매칭·동명이인은 관리자 푸시로 수동 안내. **운영자 남은 작업**: ① 하나은행 기업뱅킹에서 문자통지 신청(수신번호=공기계 유심 번호) ② 공기계에 SmsForwarder(GitHub pgyer/SmsForwarder) 설치 → 웹훅 발송채널(URL `https://progen.ai.kr/api/sms-hook`, POST, JSON `{"secret":"<SMS_HOOK_SECRET>","content":"[문자내용]"}`) + 전달규칙(전체 문자) 설정, 배터리 최적화 제외·자동시작 허용·상시 충전 ③ Vercel Production에 `SMS_HOOK_SECRET` 등록. **한계(수용함)**: 공기계가 꺼지면 문자가 서버로 안 와서 조용히 누락됨 — 미입금 알림 시퀀스·보증금 탭 수동 토글 병행. 완전 자동화 필요 시 토스페이먼츠 가상계좌가 1순위. 이하 과거 검토 기록(재시도 방지용): 검토 기록(재시도 방지용): ① 하나은행 오픈API(B2B계좌정보조회) — 사업자 회원+은행 계약 필수, 요금 비공개(문의: openapi@hanafn.com). ② 금결원 오픈뱅킹 — 수수료는 건당 수원~수십원으로 저렴하나 핀테크 기업 심사+보안점검 필요. ③ **팝빌 계좌조회 — 웹 화면용 정액제는 월 5,500원(VAT 포함)이지만 API 연동은 별도 상품 계좌 1개당 월 100,000원**(2026-09-07 견적서 No. 202609-194, 링크허브 박주선 팀장). 9/7에 API 연동 코드까지 구현했다가 견적 확인 후 9/8 전면 제거(popbill 의존성·lib·라우트·크론·env 모두 삭제). 정액제는 환불 요청(1:1 문의). 참고로 하나은행 기업뱅킹 「빠른조회 서비스」는 신청된 상태(무해). ④ 은행 앱 푸시알림 매크로 파싱 — iOS 불가·안드로이드 배터리 최적화로 매크로 사망·알림 형식 변경 취약이라 **비추천(과거 시도 실패 이력 있음)**. 향후 행사 규모가 커져 자동화가 다시 필요하면 **토스페이먼츠 가상계좌**(월 고정비 0·건당 300~400원·`DEPOSIT_CALLBACK` 웹훅 공식 지원·재전송 7회, 게스트별 고유 계좌라 입금자명 매칭 문제 자체가 없음)가 1순위 |
...
| 사전신청 폼 중복 방지 | **부분 해결(2026-07-09)** | 게스트 폼 제출 시 phone이 `crew_members`에 있으면 `/api/event-reg`가 409 `code:'crew_exists'`로 차단 → "크루로 등록된 번호예요" 모달로 크루 폼 안내(이중 등록 방지). **단 이미 발생한 이중 등록은 수동 정리 필요.** 과거 사례: 2026-05-18 포도 정인우(crew #194) / 2026-07-09 크루 권미진(#256, 010-2442-9837) — 8/1 게스트 신청을 크루로 정정 + 중복 게스트 레코드 삭제. `/api/apply`의 게스트→크루 자동 마이그레이션은 종전대로 크루 지원 시에만 동작 |
| 어드민 웹푸시 iOS 제약 | 브라우저 특성 | 아이폰은 사파리 「공유 → 홈 화면에 추가」로 PWA 설치한 뒤 그 앱에서 대시보드 열고 토글 켜야 푸시 수신(iOS 16.4+). 안드로이드/PC 크롬은 브라우저에서 바로 동작. 구독은 기기 단위라 폰+PC 둘 다 받으려면 각 기기에서 토글 1회씩 |
| 어드민 웹푸시 발송 범위 | 신청 1종만 | 현재 `/api/event-reg` 신청 접수 시에만 발송. 필요 시 크루 지원(`/api/apply`)·피드백 제출 등에도 `sendPushToAdmins` 1줄 추가로 확장 가능 |
| 이메일 알림 | 없음 | |
