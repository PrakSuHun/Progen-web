...
| is_public | BOOLEAN (NOT NULL, default true) | 웹사이트 공개 신청/체크인/피드백 대상 여부. **`getActiveEventId()`가 is_public=true 행사만 자동 선택** — 내부 프로젝트 회차(경쟁 PT·매칭데이·수료식 등)를 false로 두면 활성 행사 자동 선택에서 제외(어드민 드롭다운엔 계속 노출, 명시적 eventId로 통계·관리 가능). 어드민 「행사 생성」 신규 행사는 기본 true. 마이그레이션 `2026-07-08_event_is_public.sql`. 현재 false: 7/18 경쟁PT·8/21 매칭데이·8/28 웹교육·9/19 최종PT·11/7 수료식 (프로그램 내부 회차, `program_sessions.event_id` 연동) |
...
- **[lib/get-active-event.ts](lib/get-active-event.ts)**: `getActiveEventId()` — 활성 행사 결정 로직. **`is_public=true` 행사만 후보**(2026-07-08 — events에 내부 프로젝트 회차가 섞이면서 내부 회차가 활성 행사로 잘못 잡히던 문제 차단). 2026-05-01부터 과거 행사 fallback은 **PAST_FALLBACK_DAYS=7일 이내**일 때만 동작. 그 이상 지난 후 다음 공개 행사 row가 없으면 `null` 반환 → API들이 "현재 활성 행사를 찾을 수 없습니다" 안내. 운영진의 다음 달 events row 등록 누락 시 사용자가 지난 행사로 잘못 신청되는 사고 방지 목적.
