...
| 보증금 자동 입금 확인 | 수동 (자동화 검토 완료 후 보류, 2026-09-08) | 자동화 경로 4종 전수 검토 결과 전부 규모 대비 부적합해 **하나은행 입출금 문자통지(월 800원 정액, 기업계좌 지원) 보고 수동 토글**로 운영 확정. 검토 기록(재시도 방지용): ① 하나은행 오픈API(B2B계좌정보조회) — 사업자 회원+은행 계약 필수, 요금 비공개(문의: openapi@hanafn.com). ② 금결원 오픈뱅킹 — 수수료는 건당 수원~수십원으로 저렴하나 핀테크 기업 심사+보안점검 필요. ③ **팝빌 계좌조회 — 웹 화면용 정액제는 월 5,500원(VAT 포함)이지만 API 연동은 별도 상품 계좌 1개당 월 100,000원**(2026-09-07 견적서 No. 202609-194, 링크허브 박주선 팀장). 9/7에 API 연동 코드까지 구현했다가 견적 확인 후 9/8 전면 제거(popbill 의존성·lib·라우트·크론·env 모두 삭제). 정액제는 환불 요청(1:1 문의). 참고로 하나은행 기업뱅킹 「빠른조회 서비스」는 신청된 상태(무해). ④ 은행 앱 푸시알림 매크로 파싱 — iOS 불가·안드로이드 배터리 최적화로 매크로 사망·알림 형식 변경 취약이라 **비추천(과거 시도 실패 이력 있음)**. 향후 행사 규모가 커져 자동화가 다시 필요하면 **토스페이먼츠 가상계좌**(월 고정비 0·건당 300~400원·`DEPOSIT_CALLBACK` 웹훅 공식 지원·재전송 7회, 게스트별 고유 계좌라 입금자명 매칭 문제 자체가 없음)가 1순위 |
