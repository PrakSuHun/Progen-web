...
|---|-----|------------|------|----------|
| 1 | EVENT_REG_RECEIVED | `KA01TP26051106373464071STWgjtSAK` ✅승인 | 이름·프로그램명·일시·장소 | `/api/event-reg` mode=guest 즉시 (장소 비면 "추후 안내"). 본문에 보증금 계좌·3일 자동취소 안내 포함 |
...
| 11 | DEPOSIT_REMINDER | `KA01TP260906071133136qAave0oFtch` ⏳검수중 | 이름·프로그램명 | `send-deposit-reminder` (보증금 탭 「미입금 알림」 버튼, 미입금 행에만 노출, 수동). 본문: 입금 계좌 + 참석 시 전액 반환 안내. 2026-09-06 운영진이 솔라피에 직접 등록, 검수 통과 전엔 발송 거부됨 |
