...
| 4 | EVENT_D1_NOTICE | `KA01TP260907034359294lbAuExhWRIx` ✅승인 (2026-09-09 「행사 사전 공지」로 교체, 구 템플릿은 솔라피에서 삭제) | 이름·프로그램명·일시·입장시간·장소·진행내용·준비물 + 버튼 `#{url}` | `send-alimtalk-batch` template=d1 + `round`(1|2) (설정 탭 「행사 사전 공지」 1차/2차 토글, 수동). **같은 템플릿 2회 발송 운영(1차=확정 대체, 2차=행사 전날) — 회차별 중복 방지는 alimtalk_logs 발송 횟수(count>=round) 기준**. cron 미구현 |
