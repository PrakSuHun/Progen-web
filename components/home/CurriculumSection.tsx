...
export function CurriculumSection() {
  return (
    <section id="curriculum" className="py-20 md:py-32 px-5 lg:px-8 border-t border-[#e0e0e0] bg-[#fafafa]">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 mb-16 md:mb-24">
          <h2 className="text-4xl md:text-6xl font-black text-black leading-[1.05]">
            ...
            <span className="text-[#ccc]">&lsquo;시스템 설계자&rsquo;로</span>
          </h2>
        ...
        <div className="mb-20 md:mb-28">
          {phases.map((phase) => (
            <div key={phase.label} className="grid grid-cols-1 md:grid-cols-3 border-t border-[#e0e0e0] py-8 md:py-10 gap-6 md:gap-8">
              <div>
                <div className="text-[#aaa] text-xs font-bold tracking-widest uppercase mb-1.5">{phase.label}</div>
                <div className="text-black font-black text-lg md:text-xl mb-0.5">{phase.title}</div>
                <div className="text-[#999] text-sm">{phase.period}</div>
              </div>
              <div className="md:col-span-2">
                <p className="text-[#555] text-sm mb-5 leading-relaxed">{phase.desc}</p>
                <div className="space-y-2">
                  {phase.sessions.map((s) => (
                    <div key={s.date} className="flex gap-4 md:gap-6 bg-white border border-[#e0e0e0] rounded-xl px-4 md:px-5 py-3.5">
                      <span className="text-violet-500 font-black text-sm shrink-0 w-10 md:w-12 tabular-nums">{s.date}</span>
                      <div>
        ...
        {/* Benefits */}
        <div className="mb-20 md:mb-28">
          <div className="flex items-center gap-3 text-[#999] text-[11px] tracking-[0.2em] uppercase mb-10">
            <span className="w-6 h-px bg-[#ccc]" />
            Benefits
          </div>
          <h3 className="text-3xl md:text-5xl font-black text-black leading-tight mb-10">
            PROGEN과 함께하면<br />
            <span className="text-[#ccc]">얻게 될 4가지</span>
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 border-t border-l border-[#e0e0e0]">
            {benefits.map((b) => (
              <div key={b.num} className="flex gap-5 p-6 md:p-8 border-b border-r border-[#e0e0e0]">
                <span className="text-[#ddd] font-black text-xl shrink-0 tabular-nums">{b.num}</span>
                <div>
                  <h4 className="text-black font-bold mb-1.5 text-sm">{b.title}</h4>
        ...
        {/* Philosophy */}
        <div className="border-t border-[#e0e0e0] pt-16 md:pt-20">
          <div className="flex items-center gap-3 text-[#999] text-[11px] tracking-[0.2em] uppercase mb-8">
            <span className="w-6 h-px bg-[#ccc]" />
            운영 철학
          </div>
          <blockquote className="text-2xl md:text-4xl font-black text-black leading-snug max-w-3xl mb-10 md:mb-14">
            &ldquo;우리는 AI 전문가 집단이 아닙니다.<br />
            AI 도구를 활용해 문제를 해결하는 &lsquo;플랫폼&rsquo;입니다.&rdquo;
          </blockquote>
          <div className="grid grid-cols-1 md:grid-cols-2 border-t border-[#e0e0e0]">
            <div className="py-6 md:py-8 md:pr-12 md:border-r border-[#e0e0e0]">
              <p className="text-black font-bold text-sm mb-1.5">운영진</p>
              <p className="text-[#666] text-sm leading-relaxed">방향성 제시와 실전 멘토링을 통해 여러분의 시행착오를 줄여줍니다.</p>
            </div>
            <div className="py-6 md:py-8 md:pl-12 border-t md:border-t-0 border-[#e0e0e0]">
              <p className="text-black font-bold text-sm mb-1.5">참여자</p>
